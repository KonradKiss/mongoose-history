...
module.exports = function historyPlugin(schema, options) {
  ...
  schema.pre('save', function(next) {
    ...
    if(diffOnly && !this.isNew) {
      ...
      diff.__v = undefined;
      historyDoc['created_at'] = new Date();
      historyDoc['operation'] = 'update';
      historyDoc['collection'] = 'instance';
  ...
  // Listen on update
  schema.pre('update', getUpdateHook({ operation: 'update', collection: 'model' }));
  ...
  schema.pre('findOneAndUpdate', getUpdateHook({ operation: 'findOneAndUpdate', collection: 'model' }));

  schema.pre('findOneAndRemove', getUpdateHook({ operation: 'findOneAndRemove', collection: 'model' }));

  function getUpdateHook(options) {
    return function updateHook(next) {
      var d = this._update.$set || {};  // this._update && ...
      delete d.__v;
      ...
      historyDoc['data'] = d;
      historyDoc['collection'] = this.mongooseCollection.collectionName;
