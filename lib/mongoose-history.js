...
module.exports = function historyPlugin(schema, options) {
  ...
  // Listen on update
  schema.pre('update', getUpdateHook({ operation: 'update', table: 'model' }));
  ...
  schema.pre('findOneAndUpdate', getUpdateHook({ operation: 'findOneAndUpdate', table: 'model' }));

  schema.pre('findOneAndRemove', getUpdateHook({ operation: 'findOneAndRemove', table: 'model' }));
  ...
  function getUpdateHook(options) {
    return function updateHook(next) {
      ...
      historyDoc['data'] = d;
      historyDoc['table'] = this.mongooseCollection.collectionName;
